...
app.post("/zoominfo/viewContacts", (req, res) => {
	console.log("event");
	console.log("event body: ", req.body);
	const existingData = fs.readFileSync("../zoominfo/viewedContacts.json", "utf-8");
	const jsonData = JSON.parse(existingData);
	jsonData.push(req.body.data);
	const updatedData = JSON.stringify(jsonData);
	fs.writeFileSync("../zoominfo/viewedContacts.json", updatedData, "utf-8");
	res.json(`succeeded`);
});
